...
resource "ibm_is_lb_pool" "private-nlb-backend-pool" {
  name                = "${local.prefix}-private-pool-web"
  lb                  = ibm_is_lb.private-nlb.id
...
resource "ibm_is_instance_group" "zonal-ig" {
  resource_group = var.resource_group_id

  name              = "${local.prefix}-zonal-group"
...
resource "ibm_is_instance_group_manager" "zonal-igm" {
  name               = "${local.prefix}-zonal-igm"
  aggregation_window = 120
  instance_group     = ibm_is_instance_group.zonal-ig.id
...
resource "ibm_is_instance_group_manager_policy" "zonal-target-cpu" {
  name                   = "${local.prefix}-zonal-igm-policy"
  instance_group         = ibm_is_instance_group.zonal-ig.id
  instance_group_manager = ibm_is_instance_group_manager.zonal-igm.manager_id
  metric_type            = "cpu"
  metric_value           = 70
  policy_type            = "target"
}
