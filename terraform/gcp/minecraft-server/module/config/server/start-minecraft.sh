...
function run_minecraft_server() {
  /usr/bin/java -XX:+UnlockExperimentalVMOptions \
    ...
    -XshowSettings:all -XX:+PrintFlagsFinal -XX:NativeMemoryTracking=summary -Xlog:async -Xlog:gc*=debug,gc+ergo*=trace,gc+age*=trace,safepoint*:file=/tmp/minecraft.gc.log:level,tags,utctime,uptime,pid:filecount=5,filesize=100m \
