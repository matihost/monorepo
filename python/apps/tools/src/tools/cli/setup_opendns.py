...
from tools.utils.file import read_file, relative_path, write_file
...
def _parse_program_argv():
    ...
    parser.add_argument("opendns_network_label", metavar="opendns_network_label", type=str, help=_NETWORK_LABEL_HELP)
    parser.add_argument("-u", "--user", metavar="user", type=str, required=True, help="OpenDNS username")
    parser.add_argument("-p", "--password", metavar="password", type=str, required=True, help="OpenDNS password")
...
def ensure_ddclient_config_setup(opendns_network_label, user, password):
    ...
    desired_config = template.render(opendns_network_label=opendns_network_label, user_email=user, password=password)
