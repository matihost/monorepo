...
kubectl config use-context minikube || echo "Minikube not present in kube context" && {
  ...
  helm upgrade --install echoserver "$(dirname "${SCRIPT_DIR}")" -n learning --set ingress.tls.crt="$(base64 -w 0 /tmp/echoserver.learning.testing.crt)" --set ingress.tls.key="$(base64 -w 0 /tmp/echoserver.learning.testing.key)"
